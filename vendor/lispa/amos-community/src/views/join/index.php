        ...
        <?php if ($model->hide_participants == 0 && $model->showParticipantWidget()) { ?>
            <?php
                ...
                if ($model->context == 'lispa\amos\events\models\Event') {
                    ...
                    if(method_exists(new \lispa\amos\events\utility\EventsUtility(), 'hasPrivilegesLoggedUser')) {
                        $showWidget = false;
                        $event = \lispa\amos\events\models\Event::findOne(['community_id' => $model->id]);
                        if($event) {
                            $showWidget = \lispa\amos\events\utility\EventsUtility::hasPrivilegesLoggedUser($event);
                        }
                    } else {
                        $showWidget = true;
                    }
