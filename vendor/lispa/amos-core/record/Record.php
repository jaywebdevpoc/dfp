            ...
            foreach ($listAttributes as $key => $attribute) {
                if (is_string($this->$key)) {
                    $config = [
                        ...
                        // Change from 'XHTML 1.0 Strict'.
                        'HTML.Allowed' => 'h1,h2,h3,h4,h5,h6,b,strong,i,em,ul,ol,li,p[style],br,span,img[width|height|alt|src],iframe[width|height|src|frameborder]',
    ...
    private function updateFrequentlyCommunityUpdate()
    {
        ...
        if (isset($moduleCommunity) && isset($moduleCwh) && !empty($moduleCwh->getCwhScope())) {
            ...
            if (isset($scope['community'])) {
                $id        = $scope['community'];
                $community = \lispa\amos\community\models\Community::findOne($id);
                if (!empty($community)) {
