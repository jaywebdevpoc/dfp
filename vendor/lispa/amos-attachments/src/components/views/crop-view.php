...
jQuery('.modal-body .tools>.rotate_{$crop->attribute}', '#cropInput_{$crop->attribute}').on('click', function() {
    var data = jQuery(this).data();
    $('.modal-body .cropper-wrapper>img', '#cropInput_{$crop->attribute}').cropper(data.method, data.option);
});
    ...
    <div class="uploadcrop attachment-uploadcrop" id="cropInput_<?= $crop->attribute; ?>">
        ...
        <div id="image-source<?= $crop->imageOptions['id'] ?>" class="row cropper-body">
            ...
            <div class="btn-group tools">
                <button type="button" class="btn btn-primary rotate_<?= $crop->attribute ?>" data-method="rotate" data-option="-90" title="Rotate Left">
                    <span class="docs-tooltip" data-toggle="tooltip" data-animation="false">
                        <?= AmosIcons::show('rotate-left', ['class' => 'btn btn-primary']); ?>
                    </span>
                </button>
                <button type="button" class="btn btn-primary rotate_<?= $crop->attribute ?>" data-method="rotate" data-option="90" title="Rotate Right">
                    <span class="docs-tooltip" data-toggle="tooltip" data-animation="false">
