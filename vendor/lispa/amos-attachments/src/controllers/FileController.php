...
class FileController extends Controller
{
    ...
    protected function checkAccess($rule, $action)
    {
        ...
        if ($action->id == 'delete') {
            ...
            //User can read?
            if (!\Yii::$app->user->can($readPremission, ['model' => $model]) && (\Yii::$app->user->can($readPremissionAttribute, ['model' => $model]))) {
                return false;
            }
        }
    ...
    public function getCroppedImage($file, $cropSettings)
    {
        ...
        $cropPath = $fileDir . $file->hash . '.' . (!empty($cropSettings['width'])? $cropSettings['width'] : '') . '.' . (!empty($cropSettings['height'])? $cropSettings['height']: '') . '.' . $file->type;
