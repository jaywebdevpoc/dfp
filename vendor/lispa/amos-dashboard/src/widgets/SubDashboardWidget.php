                    ...
                    if ($widgets->count()) {
                        foreach ($widgets->all() as $value) {
                            if ($value->classname != 'lispa\amos\admin\widgets\icons\WidgetIconUserProfile' &&
                                ...
                                == false) || $this->model->className() != '\lispa\amos\community\models\Community'
                                || ($value->classname != '\lispa\amos\projectmanagement\widgets\icons\WidgetIconprojects'
                                /* && $value->classname !=  '\lispa\amos\documenti\widgets\icons\WidgetIconDocumentiDashboard' */)
                                || ( $value->classname == '\lispa\amos\projectmanagement\widgets\icons\WidgetIconprojects'
    ...
    protected function runHtml($widgets)
    {
        ...
        if ($widgets->count()) {
            if ($this->graphicCustomSize) {
                foreach ($widgets->all() as $value) {
                    if($this->model->className() != '\lispa\amos\community\models\Community'
                                   || ($value->classname !=  '\lispa\amos\documenti\widgets\graphics\WidgetGraphicsUltimiDocumenti')
                                    || ($value->classname ==  '\lispa\amos\documenti\widgets\graphics\WidgetGraphicsUltimiDocumenti'
                                           && ($this->model->id != 2751)))
                            {
