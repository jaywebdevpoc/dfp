...
class UserNetworkWidgetOrganizzazioni extends Widget
{
    ...
    public function run()
    {
        ...
        $actionColumnsButtons = [
            'deleteRelation' => function ($url, $model) use ($loggedUser) {
                ...
                $btnDelete = '';
                if (
                    (($loggedUser->id == $this->userId) && (($model->profilo->created_by != $loggedUser->id) || $loggedUser->can('AMMINISTRATORE_ORGANIZZAZIONI'))) ||
