 ...
 * @package lispa\amos\organizzazioni\models
 */
class Profilo extends \lispa\amos\organizzazioni\models\base\Profilo implements OrganizationsModelInterface, CommunityContextInterface
{
    const
        ORGANIZZAZIONI_MANAGER = 'ORGANIZZAZIONI_MANAGER',
        ORGANIZZAZIONI_PARTICIPANT = 'ORGANIZZAZIONI_PARTICIPANT';
...
//        $columns[] = 'referenteOperativo';
        ...
        $columns[] = [
            ...
            'template' => $template,
            'buttons' => [
                'update' => function ($url, $model) use ($user_id) {
                    if (
                        (in_array($user_id, array($model->rappresentante_legale, $model->referente_operativo)))
                        || (Yii::$app->user->can('ADMIN', ['model' => $model]))
                        || ((Yii::$app->user->can('MANAGER_ORGANIZZAZIONE', ['model' => $model])))
                    ) {
                        return \yii\helpers\Html::a(
                            \lispa\amos\core\icons\AmosIcons::show('edit'),
                            ['/organizzazioni/profilo/update', 'id' => $model->id],
                            [
                ...
                'community' => function ($url, $model) use ($user_id) {
                    $url = '';
                    if (is_null($model->community_id)) {
                        if (in_array($user_id, [$model->rappresentante_legale, $model->referente_operativo])) {
                            $url = \yii\helpers\Html::a(
                                \lispa\amos\core\icons\AmosIcons::show('globe-lock'),
                                ['/' . Module::getModuleName() . '/profilo/create-community/', 'id' => $model->id],
                                [
                                    'class' => 'btn btn-tools-secondary',
                        ...
                        foreach ($model->communityUserMm as $userCommunity) { // User not yet subscribed to the event
                            if ($userCommunity->user_id == $user_id) {
                                $userInList = true;
                        ...
                        if ($userInList === true) {
                            ...
                            switch ($userStatus) {
                                ...
                                case CommunityUserMm::STATUS_ACTIVE:
                                    $url = \yii\helpers\Html::a(
                                        \lispa\amos\core\icons\AmosIcons::show('globe'),
