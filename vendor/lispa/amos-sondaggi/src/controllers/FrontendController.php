...
use lispa\amos\sondaggi\utility\SondaggiUtility;
use lispa\amos\sondaggi\widgets\icons\WidgetIconSondaggiGeneral;
use kartik\mpdf\Pdf;
use Yii;
use lispa\amos\sondaggi\assets\ModuleRisultatiFrontendAsset;
...
use lispa\amos\socialauth\controllers\SocialAuthController;
...
//                foreach ($domandeWithFilesModels as $doma
                if ($completato) {
                    ...
                    $this->sendEmailSondaggioCompilato($this->model, $idSessione, $path, $user['user'],
                        ($this->model->abilita_registrazione ? null : $dati_utente), $user['new']);

                    return $this->render($thankYouPage,
                            ['url' => $url, 'pubblicazioni' => $this->model->getSondaggiPubblicaziones(), 'forzato' => $this->model->forza_lingua]);
                } else {
    ...
    protected function registerUser($sondaggioId, $idSessione)
    {
        ...
        if (!empty($model->mail_conf_community_id)) {
            $community = \lispa\amos\community\models\Community::findOne($model->mail_conf_community_id);
        }
...
//         creo un nuovo utente
                                                ...
                                                $new = false)
    {
        ...
        if (!empty($compilatore)) {
            if ($new == true) {
                ...
                $message = $model->mail_contenuto_nuovo_utente;
            } else {
                $from    = $model->mail_mittente_utente_presente;
                $subject = $model->mail_soggetto_utente_presente;
                $message = $model->mail_contenuto_utente_presente;
            }
    ...
    public function sendMailNuovoUtenteCustom($email, $model, $user = null)
    {
        ...
        if (isset($mailModule)) {
            ...
            $text    = $model->mail_registrazione_corpo;
    ...
    protected function getCampiMappati($model, $idSessione)
    {
        ...
        $campi    = \lispa\amos\sondaggi\models\SondaggiMap::find()->asArray()->all();
        $campiArr = [];
        foreach ($campi as $v) {
...
//        ])->one();
    ...
    {
        $preference = \lispa\amos\translation\models\TranslationUserPreference::find()->andWhere(['user_id' => $user_id]);
        if ($preference->count()) {
            $model       = $preference->one();
            $model->lang = $language;
            $model->save(false);
        } else {
            $newPreference          = new \lispa\amos\translation\models\TranslationUserPreference();
    ...
    public function registerToCommunity($community, $user)
    {
        if ($community) {
            ...
            if ($moduleCommunity) {
                $count = \lispa\amos\community\models\CommunityUserMm::find()->andWhere(['user_id' => $user->id,
                        'community_id' => $community->id])->count();
                if ($count == 0) {
                    $moduleCommunity->createCommunityUser($community->id,
                        \lispa\amos\community\models\CommunityUserMm::STATUS_ACTIVE,
                        \lispa\amos\community\models\CommunityUserMm::ROLE_PARTICIPANT, $user->id);
                    return true;
                }
            }
        }
...
//                    pr(implode(',', $risposteArray), 'D. ' . $domanda->id);
    ...
    {
        $content = $this->renderPartial('@vendor/lispa/amos-sondaggi/src/views/sondaggi/_view_pdf',
            [
            'data' => $data,
