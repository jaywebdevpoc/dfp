...
class SondaggiDomande extends \lispa\amos\sondaggi\models\base\SondaggiDomande
{
    ...
    public function getTutteDomandeDellaPagina()
    {
        ...
        $preQuery  = (new \yii\db\Query())->from('sondaggi_risposte_predefinite as R')->where(['IN', 'sondaggi_domande_id',
            $arrDomande]);
        $newExp = new \yii\db\Expression("D.id as domandaid, concat(D.id, ' - ', D.domanda) as domanda, R.id, R.risposta, R.ordinamento");
    ...
    {
        return $this->hasOne(\lispa\amos\sondaggi\models\SondaggiDomande::className(),
        ...
        if ($dopo > 0 && $dopo != NULL && $tipo == 'dopo') {
            if ($condizionata == 0) {
                ...
                if ($ordDopo > $ordCond) {
                    ...
                    $DomandeDopo       = $this->getTutteDomandeSondaggio()->andWhere(['>', 'ordinamento', $ordCond])->andWhere([
                        '!=', 'id', $this->id]);
